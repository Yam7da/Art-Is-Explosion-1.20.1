...
import net.yam7da.artisexplosion.entity.IctuDeCaeloEntity;
import net.yam7da.artisexplosion.entity.ArtIsExplosionEntities;
import net.yam7da.artisexplosion.item.ArtIsExplosionItems;

public class IctuDeCaeloBlock extends Block {

    public IctuDeCaeloBlock(Settings settings) {
        super(settings);
    }
    ...
    private static void prime(World world, BlockPos pos, LivingEntity igniter) {
        if (!world.isClient && world.getBlockState(pos).getBlock() instanceof IctuDeCaeloBlock) {
            IctuDeCaeloEntity entity = ArtIsExplosionEntities.ICTU_DE_CAELO.create(world);
